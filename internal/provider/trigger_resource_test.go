...
func TestAccTriggerResourceExpectedFailureOldFormat(t *testing.T) {
	re, err := regexp.Compile(".*Missing required argument.*")
	if err != nil {
		t.FailNow()
	}
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			// Create and Read testing
			{
				Config:      testAccTriggerResourceConfigExpectedFailureOldFormat(triggerName, "61169e84-93ee-415d-8d65-ddf6dc0d2939", "fefb451c-9966-4b75-b555-d4d94d7116ef"),
				ExpectError: re,
			},
			// Delete testing automatically occurs in TestCase
		},
...
func testAccTriggerResourceConfigExpectedFailureOldFormat(name, project_id, pipeline_id string) string {
	return fmt.Sprintf(`
resource "circleci_trigger" "test_trigger" {
  project_id 				= %[2]q
  pipeline_id 				= %[3]q
